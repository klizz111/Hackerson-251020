...
class AuthRoutes:
    ...
    def _register_routes(self):
        ...
        def register_ecc_():
            ...
            res = auth_service.register_ecc(username, int(data.get('pk_x')), int(data.get('pk_y')), int(data.get('c')), int(data.get('z')))
            
            if res['success']:
                return jsonify(res)
            else:
                status_code = 409 if 'already exists' in res['message'] else 400 if 'Invalid proof' in res['message'] else 500
                return jsonify(res), status_code
            
        @self.app.route('/api/login_ecc', methods=['POST'])
        def login_ecc():
            """ECC登录"""
            data = request.get_json()
            username = data.get('username')
